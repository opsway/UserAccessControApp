...
class ChefServer {
	 ...
	 */
	public function __construct($server, $endpoint, $userid, $keyfile = '/etc/chef/client.key'){
		$this->con = curl_init();
		$this->url = $server;
	...
	private function getHeaders($method, $uri, $content = ''){
		$aHeaders = $this->getAuthorizationHeaders($this->signCanonicalHeaders($method, $this->endpoint . $uri, $content));
	...
	private function setDst($uri){
		curl_setopt($this->con, CURLOPT_URL, $this->getUrl() . $uri);
	}
	...
	public function get($uri, $params = ''){
        ...
        $response = $this->execute($headers);
        if ($response === FALSE){
            $response = '{'. curl_errno($this->con). ': "' . curl_error($this->con). '"}';
        }
